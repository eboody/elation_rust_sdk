...
mod tests {
    ...
    use models::orders::*;
    use services::orders::SleepCenterService;
    ...
    #[serial]
    #[tokio::test]
    async fn test_get_sleep_center_success() {
        ...
        // Mock the GET /sleep_centers/{id}/ endpoint
        let center_id = 140755855671306;
        let sleep_center = get_mock_sleep_center(center_id);

        let mock = server.mock(|when, then| {
            when.method(GET)
                .path(format!("/sleep_centers/{}/", center_id));
            then.status(200)
                .header("Content-Type", "application/json")
                .body(serde_json::to_string(&sleep_center).unwrap());
        });

        // Create a client pointing to the mock server
        let client = Client::new().await.unwrap();
        let service = SleepCenterService::new(&client);
        ...
        assert_eq!(center.location_name, "Test Sleep Center");
    ...
    #[serial]
    #[tokio::test]
    async fn test_find_sleep_centers_success() {
        // Start a local mock server
        let server = MockServer::start_async().await;
        ...
        let mock_center = get_mock_sleep_center(140755855671306);

        let centers = vec![mock_center.clone()];
        let centers_json = serde_json::to_string(&centers).unwrap();

        // Mock the GET /sleep_centers/ endpoint with query parameters
        let mock = server.mock(|when, then| {
            when.method(GET)
                .path("/sleep_centers/")
                .query_param("location_name", "Test Sleep Center");
        ...
        let query_params = SleepCenterQueryParams {
            location_name: Some("Test Sleep Center".to_string()),
            ..Default::default()
        };
        ...
        assert_eq!(centers[0].location_name, "Test Sleep Center");
    ...
    #[serial]
    #[tokio::test]
    async fn test_get_sleep_center_not_found() {
        // Start a local mock server
        let server = MockServer::start_async().await;
        ...
        // Mock the GET /sleep_centers/{id}/ endpoint to return 404
    ...
    fn get_mock_sleep_center(center_id: i64) -> SleepCenter {
        SleepCenter {
            id: center_id,
            ...
            location_name: "Test Sleep Center".to_string(),
