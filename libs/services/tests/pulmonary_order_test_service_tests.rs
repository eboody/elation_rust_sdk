...
mod tests {
    ...
    use models::orders::*;
    use services::orders::PulmonaryOrderTestService;
    ...
    #[serial]
    #[tokio::test]
    async fn test_get_pulmonary_order_test_success() {
        ...
        // Mock the GET /pulmonary_order_tests/{id}/ endpoint
        let test_id = 140756665106487;
        let pulmonary_order_test = get_mock_pulmonary_order_test(test_id);

        let mock = server.mock(|when, then| {
            when.method(GET)
                .path(format!("/pulmonary_order_tests/{}/", test_id));
            then.status(200)
                .header("Content-Type", "application/json")
                .body(serde_json::to_string(&pulmonary_order_test).unwrap());
        });

        // Create a client pointing to the mock server
        let client = Client::new().await.unwrap();
        let service = PulmonaryOrderTestService::new(&client);
    ...
    #[serial]
    #[tokio::test]
    async fn test_find_pulmonary_order_tests_success() {
        // Start a local mock server
        let server = MockServer::start_async().await;
        ...
        let mock_test = get_mock_pulmonary_order_test(140756665106487);

        let tests = vec![mock_test.clone()];
        let tests_json = serde_json::to_string(&tests).unwrap();

        // Mock the GET /pulmonary_order_tests/ endpoint with query parameters
        let mock = server.mock(|when, then| {
            when.method(GET)
                .path("/pulmonary_order_tests/")
        ...
        let query_params = PulmonaryOrderTestQueryParams {
    ...
    #[serial]
    #[tokio::test]
    async fn test_create_pulmonary_order_test_success() {
        // Start a local mock server
        let server = MockServer::start_async().await;
        ...
        let test_for_create = PulmonaryOrderTestForCreate {
            code: Some("POT123".to_string()),
            name: "New Test".to_string(),
            practice: 140756660256772,
        };

        let created_test = PulmonaryOrderTest {
            id: 140756665106488,
            code: Some("POT123".to_string()),
            name: "New Test".to_string(),
        ...
        // Mock the POST /pulmonary_order_tests/ endpoint
        let mock = server.mock(|when, then| {
            when.method(POST)
                .path("/pulmonary_order_tests")
                .header("Content-Type", "application/json")
    ...
    #[serial]
    #[tokio::test]
    async fn test_delete_pulmonary_order_test_success() {
        // Start a local mock server
        let server = MockServer::start_async().await;
        ...
        // Mock the DELETE /pulmonary_order_tests/{id}/ endpoint
        let test_id = 140756665106487;
        let mock = server.mock(|when, then| {
            when.method(DELETE)
                .path(format!("/pulmonary_order_tests/{}/", test_id));
    ...
    #[serial]
    #[tokio::test]
    async fn test_get_pulmonary_order_test_not_found() {
        // Start a local mock server
        let server = MockServer::start_async().await;
        ...
        // Mock the GET /pulmonary_order_tests/{id}/ endpoint to return 404
        let test_id = 999999;
        let mock = server.mock(|when, then| {
            when.method(GET)
                .path(format!("/pulmonary_order_tests/{}/", test_id));
    ...
    fn get_mock_pulmonary_order_test(test_id: i64) -> PulmonaryOrderTest {
        PulmonaryOrderTest {
            id: test_id,
            code: None,
            name: "test".to_string(),
